...
def location_centroid(user_id) -> list:
    ...
    try:
        ...
        centroid = [
            sum(summed_coordinates[0]) / len(summed_coordinates[0]),
            sum(summed_coordinates[1]) / len(summed_coordinates[1]),
        ]
        return centroid
    except Exception:
        return f"No user with id of {user_id} was found"
...
def distance_from_centroid(user_id, transaction_id) -> float:
    ...
    try:
        ...
        distance_from_centroid = (
            (transaction_location[0] - centroid[0]) ** 2
            + (transaction_location[1] - centroid[1]) ** 2
        ) ** 0.5
...
def distance_between_transaction_user(user_id) -> float:
    ...
    try:
        ...
        distance = (
            (ids_coordinates[0][0] - ids_coordinates[1][0]) ** 2
            + (ids_coordinates[0][1] - ids_coordinates[1][1]) ** 2
        ) ** 0.5
        return distance
    except Exception:
        return f"An exception occurred, either due to wrong user id or transaction id does not match that particular user"
